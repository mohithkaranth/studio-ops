...
export default async function BookingCreatedMonthReportPage({
  searchParams,
}: {
  ...
  const baseHref = `/acuity/dashboard/booking-created-month?month=${monthQuery}`;
  ...
  const appointments =
    limit === null
      ? await sql<AppointmentRow[]>`
          ...
          WHERE created_datetime >= ${range.start}
            AND created_datetime < ${range.end}
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="mx-auto w-full max-w-7xl space-y-8 px-6 py-12 sm:px-10 lg:px-12">
        <header className="space-y-3">
          ...
          <div>
            <h1 className="text-3xl font-semibold tracking-tight text-zinc-50 sm:text-4xl">
              Bookings by booking created month
            </h1>
            <p className="mt-2 text-sm text-zinc-400">
              Booking records created in{" "}
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <SummaryCard title="Bookings created" value={totalCount} />
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900/70">
          <div className="flex flex-col gap-4 border-b border-zinc-800 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-lg font-medium text-zinc-50">Booking records</h2>
                ...
                {appointments.length === 0 ? (
                  <tr>
                    <td colSpan={10} className="px-5 py-8 text-center text-zinc-400">
                      No bookings found for this created month.
                    </td>
                  </tr>
                ) : (
                  appointments.map((row) => (
                    <tr
                      key={
                        row.acuity_appointment_id ?? `${row.created_datetime}-${row.client_email}`
                      }
