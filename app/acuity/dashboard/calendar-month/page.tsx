...
export default async function CalendarMonthReportPage({
  searchParams,
}: {
  ...
  const calendar = decodeURIComponent(params.calendar ?? "").trim();
  const limit = getLimit(params.limit);
  const monthQuery = encodeURIComponent(params.month ?? "");
  const calendarQuery = encodeURIComponent(params.calendar ?? "");
  const baseHref = `/acuity/dashboard/calendar-month?month=${monthQuery}&calendar=${calendarQuery}`;
    ...
    return (
      <main className="min-h-screen bg-zinc-950 text-zinc-100">
        <div className="mx-auto w-full max-w-7xl space-y-6 px-6 py-12 sm:px-10 lg:px-12">
          ...
          <p className="text-sm text-red-300">Invalid or missing month/calendar.</p>
        </div>
      </main>
    );
  }
    ...
    limit === null
      ? await sql<AppointmentRow[]>`
          ...
          WHERE appointment_datetime >= ${range.start}
            ...
            AND coalesce(nullif(calendar_name, ''), 'Unknown calendar') = ${calendar}
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="mx-auto w-full max-w-7xl space-y-8 px-6 py-12 sm:px-10 lg:px-12">
        <header className="space-y-3">
          ...
          <div>
            <h1 className="text-3xl font-semibold tracking-tight text-zinc-50 sm:text-4xl">
              Bookings by calendar name per appointment month
            </h1>
            <p className="mt-2 text-sm text-zinc-400">
              Appointment records for <span className="font-medium text-zinc-200">{calendar}</span>{" "}
              in <span className="font-medium text-zinc-200">{range.label}</span>.
            </p>
          </div>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-4">
          ...
          <SummaryCard title="Calendar" value={calendar} />
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900/70">
          <div className="flex flex-col gap-4 border-b border-zinc-800 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-lg font-medium text-zinc-50">Calendar appointment records</h2>
                ...
                {appointments.length === 0 ? (
                  <tr>
                    <td colSpan={10} className="px-5 py-8 text-center text-zinc-400">
                      No appointments found for this calendar month.
                    </td>
                  </tr>
                ) : (
