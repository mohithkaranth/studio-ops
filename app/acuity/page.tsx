...
type ReportKey = "appointment-date" | "booking-date" | "top-clients" | "cancellations";
...
export default async function AcuityPage(props: { searchParams: Promise<SearchParams> }) {
  ...
  const report: ReportKey =
    rawReport === "booking-date" || rawReport === "top-clients" || rawReport === "cancellations"
      ? rawReport
      : "appointment-date";
            ...
            <table className="min-w-full divide-y divide-zinc-800 text-left text-sm">
              ...
              <tbody className="divide-y divide-zinc-800 text-zinc-200">
                {detailRows.length === 0 ? (
                  ...
                  detailRows.map((row) => (
                    <tr key={`${row.c1}-${row.c2}`} className="hover:bg-zinc-900">
